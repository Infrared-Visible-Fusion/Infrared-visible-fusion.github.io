...
class ReviewNav0 extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      phoneOpen: false,
  ...
  render() {
    ...
    const navChildren = Object.keys(navData).map((key, i) => {
      ...
      return React.createElement(tag, { ...item, ...tagProps, key: i.toString() }, navData[key].children);
    });
      ...
      >
        <div {...dataSource.page} className={`${dataSource.page.className}${phoneOpen ? ' open' : ''}`}>
          <TweenOne animation={{ x: -30, type: 'from', ease: 'easeOutQuad' }} {...dataSource.logo}>
            <img width="100%" src={dataSource.logo.children} alt="IVIF logo" />
          </TweenOne>
          {isMobile && (
            <div
